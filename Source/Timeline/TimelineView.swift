...
public class TimelineView: UIView, ReusableView {
    ...
    var timeFont: UIFont {
        return style.timeFont
    }
    
    var verticalDiff: CGFloat = 45
    var verticalInset: CGFloat = 10
    var leftInset: CGFloat = 75
    
    var horizontalEventInset: CGFloat = 3
    
    var fullHeight: CGFloat {
        return verticalInset * 2 + CGFloat(times.count)*verticalDiff
    }
...
//    var fontSize: CGFloat = 11
    ...
    public func initHeight(){
        frame.size.height = fullHeight
    }
    
    var times: [String] {
        
        if let min = minTime, let max = maxTime{
            return timeStringsByMaxMin()
        }else{
    ...
    func timeStringsByMaxMin() -> [String] {
        var numbers = [String]()
        
        guard let min = minTime, let max = maxTime else{
            return is24hClock ? _24hTimes : _12hTimes
        }
        var cur = min
        while cur <= max{
            numbers.append(formatTimeString(cur))
            cur = cur.add(TimeChunk.init(seconds: 0, minutes: style.timelineDuration, hours: 0, days: 0, weeks: 0, months: 0, years: 0))
        }
    ...
    override public func draw(_ rect: CGRect) {
        ...
        for (i, time) in times.enumerated() {
            ...
            let timeRect = CGRect(x: 2, y: iFloat * verticalDiff + verticalInset - 7,
                                  width: leftInset - 8, height: timeFont.pointSize + 2)
    ...
    fileprivate func dateToY(_ date: Date) -> CGFloat {
        if date.dateOnly() > self.date.dateOnly() {
            ...
            let hourY = CGFloat(date.hour -  (minTime?.hour ?? 0)) * CGFloat(60/style.timelineDuration) * verticalDiff + verticalInset
            let minuteY = CGFloat(date.minute - (minTime?.minute ?? 0)) * verticalDiff / CGFloat(style.timelineDuration)
